...
function XMLParseDebutElement($parser, $name, $attrs)
{
 ...
 if($name=='NOTEBOOK')
 {
   ...
   {
     $j=$i+1;
     if($lastNB[0]==$j) $on='On'; else $on='Off';
 ...
 if($name=='PAGE')
 {
   ...
   if($lastNB[0]==$Xn[PAGE]) $Xstring.='true'; else $Xstring.='none';
 ...
 if($name=='CLIST')
 {
  ...
  for($i=0; $i<$Xoptions[COLUMNS]; $i++) $Xcliststring.='<td style="width:6px; height:1px;"></td><td id="td'.$i.'" style="width:'.($w[$i]-12).'px; height:1px;" align="'.trim($Xtemp[0][$i]).'"></td><td style="width:6px; height:1px;"></td>';
  $Xcliststring.='</tr>';
...
function XMLparseFinElement($parser, $name)
{
 ...
 if($name=='NOTEBOOK') $Xstring.='</td><td class="pagebd"></td></tr><tr><td class="pagec"></td><td class="pagecd"></td><td class="paged"></td></tr></table>';
